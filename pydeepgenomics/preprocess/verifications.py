...
#         os.path.join(settings.PATHENCODED, chrom_to_be_tested),
# extension=".txt.gz")
...
#             os.path.join(PATHORIGIN, chrom_to_be_tested,
# name+"_"+name+".txt.gz"),
#             index_col=None,
#             header=None)
#         _meta["totest"] = pd.read_csv(testfile,  index_col = None, header =
# None)
#         for i in range(nbtests):
#             totest = random.choice(_meta.totest.tolist())
#             A1, A2, position = uf.decode_position(totest, LN)
#             if position == -1:
#                 index = _meta.loc[(_meta.totest == totest),:].index.tolist(
# )[0]
...
#                 errors_next_pos.append(_meta.iloc[min(index+1, _meta.shape[
# 0]), 0])
#                 if not settings.LOGGING:
#                     uf.print_progress(j*nbtests+i,nbtests*min(nbfilesmax,
# len(files))-1, decimals = 3)
#                 if VERBOSE:
#                     print("{0}/{1} files tested. Date : {2}".format(i,
# nbtests, str(datetime.datetime.now())))
#                 continue
#             originalalleles = _meta.loc[(_meta.totest == totest),
# :]["originaldata"].tolist()[0].split("/")
#             originalpos = _meta.loc[(_meta.totest == totest),
# :]["POS"].tolist()[0]
...
#                 errors_next_pos.append(_meta.iloc[min(index+1, _meta.shape[
# 0]), 0])
#             if ((originalalleles[0] == 0) and (A1 != ref)) or ((
# originalalleles[0] == 1) and (A1 != alt)) :
#                 index = _meta.loc[(_meta.totest == totest),:].index.tolist(
# )[0]
#                 errors_file.append(testfile)
#                 errors_sup_pos.append(position)
...
#                 errors_next_pos.append(_meta.iloc[min(index+1, _meta.shape[
# 0]), 0])
#             if ((originalalleles[-1] == 0) and (A1 != alt)) or ((
# originalalleles[-1] == 1) and (A1 != alt)) :
#                 index = _meta.loc[(_meta.totest == totest),:].index.tolist(
# )[0]
#                 errors_file.append(testfile)
#                 errors_sup_pos.append(position)
...
# len(files))-1, decimals = 3)
#             if VERBOSE:
#                 print("{0}/{1} files tested. Date : {2}".format(i, nbtests,
...
#         Impossibletodecode = errors.loc[(errors.Error_type == "Impossible to
#  decode"),:].shape[0]
...
#         print("In total : {}% errors !\n".format(100*totalerror/(
# nbtests*min(nbfilesmax, len(files)))))
#         print("Date : {}".format(str(datetime.datetime.now())))
#         errors.to_csv("Errorsfoundin{}.csv".format(chrom_to_be_tested),
# sep ="\t")
#     else :
